...
MainToolBar::MainToolBar(std::weak_ptr<wui::window> mainFrame_, IControlActions &receiver)
    : parent(mainFrame_),
    panel(std::make_shared<wui::panel>("toolbar_panel")),
    call          (std::make_shared<wui::button>(wui::locale("toolbar", "call"),                [&receiver]() { receiver.ActionCall();                  }, wui::button_view::image, IMG_TB_CALL,                        BTN_SIZE, wui::button::tc_tool)),
    conference    (std::make_shared<wui::button>(wui::locale("toolbar", "conference"),          [&receiver]() { receiver.ActionConference();            }, wui::button_view::image, IMG_TB_CONFERENCE,                  BTN_SIZE, wui::button::tc_tool)),
    hangup        (std::make_shared<wui::button>(wui::locale("toolbar", "hangup"),              [&receiver]() { receiver.ActionHangup();                }, wui::button_view::image, IMG_TB_HANGUP,                      BTN_SIZE, wui::button::tc_tool)),
    separator0    (std::make_shared<wui::image>(IMG_TB_SEPARATOR)),
    camera        (std::make_shared<wui::button>(Params(Button::Camera).tooltip,                [&receiver]() { receiver.ActionTurnCamera(true);        }, wui::button_view::image, Params(Button::Camera).image,       BTN_SIZE, wui::button::tc_tool)),
    microphone    (std::make_shared<wui::button>(Params(Button::Microphone).tooltip,            [&receiver]() { receiver.ActionTurnMicrophone(true);    }, wui::button_view::image, Params(Button::Microphone).image,   BTN_SIZE, wui::button::tc_tool)),
    microphoneVol (std::make_shared<wui::button>(wui::locale("toolbar", "gain"),                [&receiver]() { receiver.ActionVolumeMicrophone();      }, wui::button_view::image, IMG_TB_EXPAND,                      EXP_SIZE, wui::button::tc_tool)),
    speaker       (std::make_shared<wui::button>(Params(Button::Speaker).tooltip,               [&receiver]() { receiver.ActionTurnLoudspeaker();       }, wui::button_view::image, Params(Button::Speaker).image,      BTN_SIZE, wui::button::tc_tool)),
    speakerVol    (std::make_shared<wui::button>(wui::locale("toolbar", "volume"),              [&receiver]() { receiver.ActionVolumeLoudspeaker();     }, wui::button_view::image, IMG_TB_EXPAND,                      EXP_SIZE, wui::button::tc_tool)),
    separator1    (std::make_shared<wui::image>(IMG_TB_SEPARATOR)),
    screenCapturer(std::make_shared<wui::button>(wui::locale("toolbar", "screen_capturer_off"), [&receiver]() { receiver.ActionTurnDemonstration(true); }, wui::button_view::image, IMG_TB_SCREENCAPTURE_DISABLED,      BTN_SIZE, wui::button::tc_tool)),
    separator2    (std::make_shared<wui::image>(IMG_TB_SEPARATOR)),
    rendererMode  (std::make_shared<wui::button>(Params(Button::RendererMode).tooltip,          [&receiver]() { receiver.ActionTurnRendererGrid();      }, wui::button_view::image, Params(Button::RendererMode).image, BTN_SIZE, wui::button::tc_tool)),
    separator3    (std::make_shared<wui::image>(IMG_TB_SEPARATOR)),
    hand          (std::make_shared<wui::button>(wui::locale("toolbar", "hand"),                [&receiver]() { receiver.ActionHand(true);              }, wui::button_view::image, IMG_TB_HAND,                        BTN_SIZE, wui::button::tc_tool)),
    separator4    (std::make_shared<wui::image>(IMG_TB_SEPARATOR)),
    list          (std::make_shared<wui::button>(Params(Button::List).tooltip,                  [&receiver]() { receiver.ActionTurnListPanel();         }, wui::button_view::image, Params(Button::List).image,         BTN_SIZE, wui::button::tc_tool)),
    content       (std::make_shared<wui::button>(Params(Button::Content).tooltip,               [&receiver]() { receiver.ActionTurnContentPanel();      }, wui::button_view::image, Params(Button::Content).image,      BTN_SIZE, wui::button::tc_tool)),
